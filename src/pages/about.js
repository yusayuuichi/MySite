...
const About = () => {
    return (
      ...
      但算是第一次寫前端測試，所以大概會寫得很醜`}
        ...
        還未把履歷編輯頁給寫完`}
            />
            <h3 className='mt-3'>20221014想法</h3>
            <TextShow
                text={`工作結束後又碰上新租屋的簽約、連續好幾天的搬家等等，比較沒心思放在這...
        現在先將基本資料到串接 api 前完成了，又要回到 Node 上寫好 update 的 api`}
            />
            <h3 className='mt-3'>20221015想法</h3>
            <TextShow
                text={`今天完成了履歷維護區的基本資料維護流程，畢竟是寫給自己用的，越輕便越好
        接下來就是工作經歷的資料維護了，預計開始使用 react-bootstrap 的表格外，
        也需要可以新增工作經歷的流程`}
            />
            <h3 className='mt-3'>20221116想法</h3>
            <TextShow
                text={`10月中到這幾天突然工作爆量，人力少的情況下都需要加班趕工，
        加上近期主要集中在看理財跟攝影相關的書籍，以致於這邊都還排不上時間
        目前先將技能中部分僅為接觸過的、不熟的隔開來排列，代表有在嘗試自己去接觸工作外的技術
        然後將關於給放進登入後才能查看。`}
            />
        </Container>
    )
